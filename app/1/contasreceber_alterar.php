...
if (isset($LOG_CAMINHO)) {
    ...
    $identificacao = date("dmYHis") . "-PID" . getmypid() . "-" . "contasreceber_alterar";
    if (isset($LOG_NIVEL)) {
        if ($LOG_NIVEL >= 1) {
...
if (isset($jsonEntrada['idCR'])) {
    $idCR = $jsonEntrada['idCR'];
    ...
    $sql = "UPDATE `contasreceber` SET `idCliente`= $idCliente,`numeroDocumento`= '$numeroDocumento',`dataEmissao`= '$dataEmissao',`dataVencimento`='$dataVencimento',
    `idContaCorrente`= $idContaCorrente,`dataLiquidacao`='$dataLiquidacao',`idNotaServico`= $idNotaServico,`valorReceber`='$valorReceber',`valorAberto`= $valorAberto,`condicao`='$condicao', `idCategoria`='$idCategoria'
    WHERE contasreceber.idCR = $idCR";
